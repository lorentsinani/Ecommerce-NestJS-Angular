...
export class CurrencyService {
  private static CreateExceptionMessage = 'Currency is not created';
  private static FindExceptionMessage = 'Currency not found';
  ...
  async create(currencyBody: CreateCurrencyDto): Promise<ICurrency> {
    ...
    if (!createdCurrency) {
      throw new HttpException(CurrencyService.CreateExceptionMessage, HttpStatus.BAD_REQUEST);
    }
  ...
  async findAll(): Promise<ICurrency[]> {
    ...
    if (!currencyExist) {
      throw new HttpException(CurrencyService.FindExceptionMessage, HttpStatus.NOT_FOUND);
    }
