...
import {
  AngularPaginatorInstance,
  Page,
} from '../interface/paginator.interface';
...
export class AngularPaginatorDirective implements OnInit, OnDestroy {
  ...
  constructor(private angularPaginatorService: AngularPaginatorService) {
    ...
    this.subscription = this.angularPaginatorService.change.subscribe(
      (id: string) => {
        if (id === this.id) {
          this.updatePages();
        }
  ...
  private getPages(
    ...
    const pageLimits = this.computePageLimits(
      currentPage,
      totalItems,
      itemsPerPage
    );
  ...
  private updatePages(): void {
    const instance: AngularPaginatorInstance =
      this.angularPaginatorService.getInstance(this.id);

    const correctedCurrentPage = this.outOfBoundCorrection(instance);

    if (
      correctedCurrentPage !== instance.currentPage ||
